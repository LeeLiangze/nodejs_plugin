...
const createMatch = require('cmos-path-matching');
...
module.exports = (_, app) => {
  ...
  if (options.match || options.ignore) {
    app.coreLogger.warn('[cmos-security] Please set `match` or `ignore` on sub config');
  }
  ...
  defaultMiddleware.forEach(middlewareName => {
    ...
    if (opt.match && opt.ignore) {
      app.coreLogger.warn('[cmos-security] `options.match` and `options.ignore` are both set, using `options.match`');
      opt.ignore = undefined;
    }
    if (!opt.ignore && opt.blackUrls) {
      app.deprecate('[cmos-security] Please use `config.security.xframe.ignore` instead, `config.security.xframe.blackUrls` will be removed very soon');
    ...
    middlewares.push(fn);
    app.coreLogger.info('[cmos-security] use %s middleware', middlewareName);
  });
  app.coreLogger.info('[cmos-security] compose %d middlewares into one security middleware',
    middlewares.length);
