...
function getFrameworkPath({ framework, baseDir }) {
  ...
  // 1. pass framework or customCmos
  if (framework) {
  ...
  // 2.1 use framework name from pkg.cmos.framework
  if (pkg.cmos && pkg.cmos.framework) {
    return assertAndReturn(pkg.cmos.framework, moduleDir);
  }

  // 2.2 use cmos by default
  return assertAndReturn('cmos', moduleDir);
}
