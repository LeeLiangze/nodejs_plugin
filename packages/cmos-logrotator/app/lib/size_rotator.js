...
const debug = require('debug')('cmos-logrotator:size_rotator');
    ...
    for (const logPath of filesRotateBySize) {
      ...
      try {
        ...
        if (stat.size >= maxFileSize) {
          this.logger.info(`[cmos-logrotator] file ${logPath} reach the maximum file size, current size: ${stat.size}, max size: ${maxFileSize}`);
        ...
        err.message = '[cmos-logrotator] ' + err.message;
