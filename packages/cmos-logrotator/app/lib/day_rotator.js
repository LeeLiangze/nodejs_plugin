...
const debug = require('debug')('cmos-logrotator:day_rotator');
...
class DayRotator extends Rotator {
  ...
  * getRotateFiles() {
    ...
    // - app worker -> cmos-web.log
    // - agent worker -> cmos-agent.log
    ...
    if (rotateLogDirs && rotateLogDirs.length > 0) {
      this.app.deprecate('[cmos-logrotator] Do not use app.config.logger.rotateLogDirs, only rotate core loggers and custom loggers');
