...
module.exports = app => ({
  ...
  * task() {
    ...
    logger.info('[cmos-logrotator] clean all log before %s days', maxDays);
  },
});
...
function* removeExpiredLogFiles(logdir, maxDays, logger) {
  ...
  logger.info(`[cmos-logrotator] start remove ${logdir} files: ${names.join(', ')}`);
  ...
  yield names.map(name => function* () {
    ...
    try {
      ...
      err.message = `[cmos-logrotator] remove logfile ${logfile} error, ${err.message}`;
      logger.error(err);
    }
  });
}
