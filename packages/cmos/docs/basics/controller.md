...
请求 body 除了可以带参数之外，还可以发送文件，一般来说，浏览器上都是通过 `Multipart/form-data` 格式发送文件的，框架通过内置 [Multipart](https://github.com/cmosjs/cmos-multipart) 插件来支持获取用户上传的文件。
...
由于 header 比较特殊，有一些是 `HTTP` 协议规定了具体含义的（例如 `Content-Type`，`Accept`），有些是反向代理设置的，已经约定俗成（X-Forwarded-For），框架也会对他们增加一些便捷的 getter，详细的 getter 可以查看 [API](https://cmosjs.org/api/) 文档。
...
框架内置了 [Session](https://github.com/cmosjs/cmos-session) 插件，给我们提供了 `context.session` 来访问或者修改当前用户 Session 。
...
借助 [Validate](https://github.com/cmosjs/cmos-validate) 插件提供便捷的参数校验机制，帮助我们完成各种复杂的参数校验。
...
cmos 自身没有集成任何一个模板引擎，但是约定了 [View 插件的规范](../advanced/view-plugin.md)，通过接入的模板引擎，可以直接使用 `ctx.render(template)` 来渲染模板生成 html。
