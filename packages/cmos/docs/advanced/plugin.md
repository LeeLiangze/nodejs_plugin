...
和中间件不同，插件是自己管理依赖的。应用在加载所有插件前会预先从它们的 `package.json` 中读取 `cmosPlugin > dependencies` 和 `cmosPlugin > optionalDependencies` 节点，然后根据依赖关系计算出加载顺序，举个例子，下面三个插件的加载顺序就应该是 `c => b => a`
...
**注意：`dependencies` 和 `optionalDependencies` 的取值是另一个插件的 `cmosPlugin.name`，而不是 `package name`。**
...
我们将 [cmos-mysql] 的实现简化之后来看看如何编写此类插件：
...
function createMysql(config, app) {
  ...
  app.beforeStart(function* () {
    ...
    const index = count++;
    app.coreLogger.info(`[cmos-mysql] instance[${index}] status OK, rds currentTime: ${rows[0].currentTime}`);
  });
...
首先 cmos 插件不仅仅支持 npm 包，还支持通过目录来找插件。在[渐进式开发](../tutorials/progressive.md)章节提到如何使用这两个配置来进行代码演进。目录对单元测试也比较友好。所以 cmos 无法通过 npm 的包名来做唯一性。

更重要的是 cmos 可以使用这种特性来做适配器。比如[模板开发规范](./view-plugin.md#插件命名规范)定义的插件名为 view，而存在 `cmos-view-nunjucks`，`cmos-view-react` 等插件，使用者只需要更换插件和修改模板，不需要动 Controller， 因为所有的模板插件都实现了相同的 API。
