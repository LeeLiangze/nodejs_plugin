...
统一使用 [cmos-bin 来运行测试脚本](./development.md#单元测试)，
...
> unittest-example@ test /Users/mk2/git/github.com/cmosjs/examples/unittest
> cmos-bin test
...
所以我们单独为框架抽取了一个测试 mock 辅助模块：[cmos-mock](https://github.com/cmosjs/cmos-mock)，
...
通过 cmos-mock，结合 Mocha 的 `before` 钩子就可以便捷地创建出一个 app 实例。

```js
// test/controller/home.test.js
const assert = require('assert');
const mock = require('cmos-mock');
...
更多关于创建 app 的信息请查看 [`mock.app(options)`](https://github.com/cmosjs/cmos-mock#options) 文档。

### ctx

我们除了 app，还需要一种方式便捷地拿到 ctx，方便我们进行 Extend、Service、Helper 等测试。
而我们已经通过上面的方式拿到了一个 app，结合 cmos-mock 提供的 [`app.mockContext(options)`](https://github.com/cmosjs/cmos-mock#appmockcontextoptions) 方法来快速创建一个 ctx 实例。
...
cmos-bin 会自动加载 thunk-mocha 插件测试异步调用，它支持多种写法，比如上面启动完成，`app.ready()` 返回一个 Promise。
...
所以 cmos-mock 对 app 增加了 `app.mockCsrf()` 方法来模拟取 CSRF token 的过程。
...
cmos-mock 创建 app 的时候，已经将 Application 的扩展自动加载到 app 实例了，
...
cmos-mock 除了上面介绍过的 `app.mockContext()` 和 `app.mockCsrf()` 方法外，还提供了[非常多的 mock 方法](https://github.com/cmosjs/cmos-mock#api)帮助我们便捷地写单元测试。
...
下面会详细解释一下 cmos-mock 的常见使用场景。

### Mock 属性和方法

因为 cmos-mock 是扩展自 [mm](https://github.com/node-modules/mm) 模块，
...
exports.httpclient = function* (ctx) {
  const res = ctx.curl('https://cmosjs.org');
...
describe('GET /httpclient', () => {
  it('should mock httpclient response', () => {
    app.mockHttpclient('https://cmosjs.org', {
      // 模拟的参数，可以是 buffer / string / json，
      // 都会转换成 buffer
      // 按照请求时的 options.dataType 来做对应的转换
      data: 'mock cmosjs.org response',
    });
    return request(app.callback())
      .get('/httpclient')
      .expect('mock cmosjs.org response');
  });
});
```
...
完整示例代码可以在 [cmosjs/exmaples/unittest](https://github.com/cmosjs/examples/blob/master/unittest) 找到。
