...
const proto = module.exports = {
  ...
  runInBackground(scope) {
    ...
    co(function* () {
      yield scope(ctx);
      ctx.coreLogger.info('[cmos:background] task:%s success (%dms)', taskName, Date.now() - start);
    }).catch(err => {
      ctx.coreLogger.info('[cmos:background] task:%s fail (%dms)', taskName, Date.now() - start);
