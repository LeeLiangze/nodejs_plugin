...
const fs = require('fs');
const CmosCore = require('cmos-core').CmosCore;
const cluster = require('cluster-client');
const extend = require('extend2');
const ContextLogger = require('cmos-logger').CmosContextLogger;
const ContextCookies = require('cmos-cookies');
...
const HTTPCLIENT = Symbol('CmosApplication#httpclient');
const LOGGERS = Symbol('CmosApplication#loggers');
 ...
 * @extends CmosCore
 */
class CmosApplication extends CmosCore {
  ...
  constructor(options) {
    ...
    this.console.info('[cmos:core] App root: %s', this.baseDir);
    this.console.info('[cmos:core] All *.log files save on %j', this.config.logger.dir);
    this.console.info('[cmos:core] Loaded enabled plugin %j', this.loader.orderPlugins);
   ...
   *   subdomainOffset: 2,
   *   config: '<cmos config>',
   *   controller: '<cmos controller>',
   *   service: '<cmos service>',
   *   middlewares: '<cmos middlewares>',
   *   urllib: '<cmos urllib>',
   *   loggers: '<cmos loggers>'
   * }
   * ```
   */
  inspect() {
  ...
  /**
   * core logger for framework and plugins, log file is `$HOME/logs/{appname}/cmos-web`
   * @member {Logger}
   * @since 1.0.0
   */
  get coreLogger() {
...
module.exports = CmosApplication;
