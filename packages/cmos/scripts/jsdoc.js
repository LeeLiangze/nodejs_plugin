...
class Source extends Set {
  constructor() {
    ...
    this.add(path.join(cwd, 'node_modules/cmos-core/index.js'));
    this.add(path.join(cwd, 'node_modules/cmos-core/lib'));

    // this.add(path.join(cwd, 'node_modules/cmos-logger/index.js'));
    // this.add(path.join(cwd, 'node_modules/cmos-logger/lib'));
  }

  getLoadUnits() {
    const CMOS_LOADER = Symbol.for('cmos#loader');
    const CMOS_PATH = Symbol.for('cmos#cmos');
