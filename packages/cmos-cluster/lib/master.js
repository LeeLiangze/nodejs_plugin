...
class Master extends EventEmitter {
  ...
  constructor(options) {
    ...
    this.logger = new ConsoleLogger({ level: process.env.CMOS_MASTER_LOGGER_LEVEL || 'INFO' });
    this.logMethod = 'info';
    if (process.env.CMOS_SERVER_ENV === 'local' || process.env.NODE_ENV === 'development') {
    ...
    this.log('[master] start with env: isProduction: %s, isDebug: %s, CMOS_SERVER_ENV: %s, NODE_ENV: %s',
      this.isProduction, this.isDebug, process.env.CMOS_SERVER_ENV, process.env.NODE_ENV);
  ...
  onAgentExit(data) {
    ...
    this.messenger.send({ action: 'cmos-pids', to: 'app', data: [] });
  ...
  onAgentStart() {
    this.messenger.send({ action: 'cmos-pids', to: 'app', data: [ this.agentWorker.pid ] });
  ...
  onAppExit(data) {
    ...
    this.messenger.send({ action: 'cmos-pids', to: 'agent', data: getListeningWorker(this.workers) });
...
function isProduction() {
  const serverEnv = process.env.CMOS_SERVER_ENV;
  if (serverEnv) {
