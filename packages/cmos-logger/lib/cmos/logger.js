...
class CmosLogger extends Logger {
  /**
   * @constructor
  ...
  constructor(options) {
    ...
    const CmosFileTransport = this.options.buffer === true ? FileBufferTransport : FileTransport;

    const fileTransport = new CmosFileTransport({
    ...
    if (this.options.jsonFile) {
      const jsonFileTransport = new CmosFileTransport({
