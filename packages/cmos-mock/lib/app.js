...
const debug = require('debug')('cmos-mock');
...
class MockApplication extends EventEmitter {
  ...
  * [INIT]() {
    ...
    const cmos = require(this.options.framework);

    const Agent = cmos.Agent;
    const agent = this._agent = new Agent(this.options);
    debug('agent instantiate');
    yield agent.ready();
    debug('agent ready');

    const Application = bindMessenger(cmos.Application, agent);
...
function bindMessenger(Application, agent) {
  ...
  return class MeesengerApplication extends Application {
    ...
    get [Symbol.for('cmos#cmosPath')]() { return path.join(__dirname, 'tmp'); }
  };
}
