...
const debug = require('debug')('cmos-mock:cluster');
...
/**
 * A cluster version of cmos.Application, you can test with supertest
 * @example
 * ```js
 * const mm = require('mm');
  ...
  constructor(options) {
    ...
    process.nextTick(() => {
      this.proc.on('message', msg => {
        // 'cmos-ready' and { action: 'cmos-ready' }
  ...
  _callFunctionOnAppWorker(method, args = []) {
    for (let i = 0; i < args.length; i++) {
      ...
      if (typeof arg === 'function') {
        args[i] = {
          __cmos_mock_type: 'function',
          value: arg.toString(),
        };
      } else if (arg instanceof Error) {
        const errObject = {
          __cmos_mock_type: 'error',
          name: arg.name,
          message: arg.message,
          stack: arg.stack,
        };
