...
const debug = require('debug')('cmos-mock:middleware:cluster_app_mock');

module.exports = () => {
  return function* (next) {
    if (this.path !== '/__cmos_mock_call_function') return yield next;
    ...
    for (let i = 0; i < args.length; i++) {
      ...
      if (arg && typeof arg === 'object') {
        // convert __cmos_mock_type back to function
        if (arg.__cmos_mock_type === 'function') {
          // eslint-disable-next-line
          args[i] = eval(`(function() { return ${arg.value} })()`);
        } else if (arg.__cmos_mock_type === 'error') {
          const err = new Error(arg.message);
          err.name = arg.name;
          err.stack = arg.stack;
          for (const key in arg) {
            if (key !== 'name' && key !== 'message' && key !== 'stack' && key !== '__cmos_mock_type') {
              err[key] = arg[key];
            }
          }
          args[i] = err;
