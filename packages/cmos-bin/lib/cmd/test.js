...
const debug = require('debug')('cmos-bin:test');
const fs = require('fs');
...
class TestCommand extends Command {
  constructor(rawArgv) {
    super(rawArgv);
    this.usage = 'Usage: cmos-bin test [files] [options]';
    this.options = {
      require: {
  ...
  * run(context) {
  ...
  formatTestArgs({ argv, debug }) {
    ...
    if (requireArr.includes('intelli-espower-loader')) {
      console.warn('[cmos-bin] don\'t need to manually require `intelli-espower-loader` anymore');
    } else {
