...
const debug = require('debug')('cmos-bin:dev');
...
class DevCommand extends Command {
  constructor(rawArgv) {
    super(rawArgv);
    this.usage = 'Usage: cmos-bin dev [dir] [options]';
  ...
  * run(context) {
  ...
  /**
   * format cmos startCluster args then change it to json string style
   * @method helper#formatArgs
   * @param {Object} context - { cwd, argv }
   * @return {Array} pass to start-cluster, [ '{"port":7001,"framework":"cmos"}' ]
   */
  * formatArgs(context) {
    ...
    if (!argv.port) {
      ...
      if (port !== this.defaultPort) {
        argv.port = port;
        console.warn(`[cmos-bin] server port ${this.defaultPort} is in use, now using port ${port}\n`);
      }
