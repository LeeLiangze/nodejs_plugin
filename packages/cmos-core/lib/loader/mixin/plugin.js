...
const debug = require('debug')('cmos-core:plugin');
...
module.exports = {
  ...
  /**
   * Load config/plugin.js from {CmosLoader#loadUnits}
   *
   ...
   * 2. $CMOS_BASE/node_modules/${package}
   *
  ...
  loadPlugin() {
    ...
    const cmosPluginConfigPaths = this.cmosPaths.map(cmosPath => path.join(cmosPath, 'config/plugin.default.js'));
    const cmosPlugins = this.readPluginConfigs(cmosPluginConfigPaths);
    debug('Loaded cmos plugins: %j', Object.keys(cmosPlugins));

    // loader plugins from process.env.CMOS_PLUGINS
    let customPlugins;
    if (process.env.CMOS_PLUGINS) {
      try {
        customPlugins = JSON.parse(process.env.CMOS_PLUGINS);
      } catch (e) {
        debug('parse CMOS_PLUGINS failed, %s', e);
      }
    }
    ...
    this.cmosPlugins = cmosPlugins;

    this._extendPlugins(this.allPlugins, cmosPlugins);
     ...
     * @member {Object} CmosLoader#plugins
     * @since 1.0.0
     */
  ...
  mergePluginConfig(plugin) {
    ...
    if (!config) {
      logger.warn(`[cmos:loader] pkg.cmosPlugin is missing in ${pluginPackage}`);
      return;
    }

    if (config.name && config.name !== plugin.name) {
      // pluginName is configured in config/plugin.js
      // pluginConfigName is pkg.cmosPath.name
      logger.warn(`[cmos:loader] pluginName(${plugin.name}) is different from pluginConfigName(${config.name})`);
    }
  ...
  getPluginPath(plugin) {
    ...
    //    -> {CMOS_PATH}/node_modules
    //      -> $CWD/node_modules
    lookupDirs.push(path.join(this.options.baseDir, 'node_modules'));

    // 到 cmos 中查找，优先从外往里查找
    for (let i = this.cmosPaths.length - 1; i >= 0; i--) {
      const cmosPath = this.cmosPaths[i];
      lookupDirs.push(path.join(cmosPath, 'node_modules'));
    }
