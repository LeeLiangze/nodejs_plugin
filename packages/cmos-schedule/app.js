...
module.exports = app => {
  ...
  app.runSchedule = schedulePath => {
    ...
    try {
      ...
      err.message = `[cmos-schedule] ${err.message}`;
  ...
  for (const s in schedules) {
    ...
    app.coreLogger.info('[cmos-schedule]: register schedule %s', key);
    app.messenger.on(key, () => {
      app.coreLogger.info('[cmos-schedule]: get message %s', key);
      ...
      .then(() => true) // succeed
      .catch(err => {
        err.message = `[cmos-schedule] ${key} excute error. ${err.message}`;
      ...
      .then(success => {
        ...
        app.coreLogger.info(`[cmos-schedule] ${key} excute ${status}, used ${rt}ms`);
      });
    });
  }
};
